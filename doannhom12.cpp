...
void popularString(Node *dad, int &popular, string &temp)
{
    string a;
    if (dad->NodeChild.empty())
    {
        cout << "Chuoi pho bien nhat: ";
        cout << temp;
        return;
    }
    else
    ...
    for (Node *ptr : dad->NodeChild)
    {
        if (ptr->popular == popular)
        {
            popularString(ptr, popular, temp);
        }
    }
}
